...
PrependTo[$ModelPath, "model"];

inimod[mod_, opt___] := (
  SetOptions[InsertFields, Model -> mod, opt];
  InitializeModel[mod, Reinitialize -> True];
);
ifold[i_, h_:Identity] :=
  IndexRange[Index[i]] = h[Level[IndexRange[Index[i]], {-1}]];

$CKM = True;
modopt["mfv"] :=
  inimod[{"MSSMCT", "EnhHbb", "dZHmHp=0"}];
modopt["nmfv"] := (
  $FV = {11, 12, 13, 14};
  inimod[{"MSSMCT", "FV", "EnhHbb", "dZHmHp=0"}] );
modopt["gl"] := (
  CKM = CKMC = IndexDelta;
  inimod[{"MSSMCTgl" (*, "EnhHbb"*), "dZHmHp=0", "dMSfsq1z"},
    InsertionLevel -> {Particles},
    ExcludeParticles -> {F[1], S[11], F[2], S[12]},
    Restrictions -> {NoGeneration1, NoGeneration2} (*,
    LastSelections -> F[3|4,{3,_}] | S[13|14,{_,3,_}]*)];
  ifold[Sfermion, Identity] );
_modopt := Abort[];
modopt[mod];

modopt[_, i_ -> i_] := Null;
modopt[mod_, _ -> mhpinput_] :=
  ($Model = ""; $MHpInput = mhpinput; modopt[mod])
