{C8LCha -> {(SumOver[All4, 6]*SumOver[Cha4, 2]*SumOver[Ind1, 3]*
     SumOver[Ind2, 3]*((MB*SB2*(A0[MASf2[All4, 3]] - A0[MCha2[Cha4]])*
        CKM[Ind2, 3]*CKMC[Ind1, 2]*MCha[Cha4]*Mf[bTR, 3]*UASf[All4, Ind2, 3]*
        UCha[Cha4, 2]*(Sqrt[2]*MW*SB*UASfC[All4, Ind1, 3]*VCha[Cha4, 1] - 
         Mf[3, Ind1]*UASfC[All4, 3 + Ind1, 3]*VCha[Cha4, 2]))/
       (MASf2[All4, 3] - MCha2[Cha4]) + MB*SB2*B0[MB2, MASf2[All4, 3], 
        MCha2[Cha4]]*CKM[Ind2, 3]*CKMC[Ind1, 2]*MCha[Cha4]*Mf[bTR, 3]*
       UASf[All4, Ind2, 3]*UCha[Cha4, 2]*
       (-(Sqrt[2]*MW*SB*UASfC[All4, Ind1, 3]*VCha[Cha4, 1]) + 
        Mf[3, Ind1]*UASfC[All4, 3 + Ind1, 3]*VCha[Cha4, 2]) - 
      (CKM[Ind1, 3]*CKMC[Ind2, 2]*(MB2 - 2*A0[MASf2[All4, 3]] + 
         2*A0[MCha2[Cha4]] + 2*MB2*C0[MB2, 0, 0, MASf2[All4, 3], MCha2[Cha4], 
           MASf2[All4, 3]]*MASf2[All4, 3] + 2*B0[MB2, MASf2[All4, 3], 
           MCha2[Cha4]]*(MASf2[All4, 3] - MCha2[Cha4]))*
        (2*SB2*UASf[All4, Ind1, 3]*(MW2*S2B*UASfC[All4, Ind2, 3]*
            VCha[Cha4, 1] - Sqrt[2]*CB*MW*Mf[3, Ind2]*UASfC[All4, 3 + Ind2, 
             3]*VCha[Cha4, 2])*VChaC[Cha4, 1] + Mf[3, Ind1]*
          UASf[All4, 3 + Ind1, 3]*(-2*Sqrt[2]*CB*MW*SB2*UASfC[All4, Ind2, 3]*
            VCha[Cha4, 1] + S2B*Mf[3, Ind2]*UASfC[All4, 3 + Ind2, 3]*
            VCha[Cha4, 2])*VChaC[Cha4, 2]))/4))/(MB2^2*S2B*SB2*CKM[3, 3]*
     CKMC[3, 2])}}
