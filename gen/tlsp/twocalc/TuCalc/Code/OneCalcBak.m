   ...
   For[i=1, i <= lamp, i++,
	   ...
	   If[ MemberQ[double, i], 
	       ...
	       neu = Collect2[neu, {A0,B0,B0p,T,Y}] ],
               neui = amplitudes[[i]] /. FeynAmp -> doit;
	       neu = neu + (*Expand[*)neui(*]*);
	       If[fac===True,
...
(* The function which does it *)
 ...
 sq[Momentum[K1, ___], Momentum[K2, ___]]:= 1/2 (numprop[K2] + numprop[K1] - numprop[p]);
 sq[Momentum[p, ___], Momentum[K1, ___]]:=  1/2 (numprop[K2] - numprop[K1] - numprop[p]);
 sq[Momentum[p, ___], Momentum[K2, ___]]:=  1/2 (numprop[K2] - numprop[K1] + numprop[p]);
