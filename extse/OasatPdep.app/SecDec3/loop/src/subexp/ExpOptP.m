(*
  #****m* SecDec/loop/src/subexp/ExpOptP.m
  #  NAME
  #    ExpOpt.m
  #
  #  USAGE
...
Remove[subs,d];
Quiet[
Needs["Experimental`"];Evaluate[Get[StringJoin[path,"src/util/","Format.m"]]];
ParallelNeeds["Experimental`"];ParallelEvaluate[Get[StringJoin[path,"src/util/","Format.m"]]]
];
...
writeopt[inlist_,varletter_,funcname_,outfile_]:=
  Block[{li1,li2,count,file,le,helpstring,fu,repl,varname,varval,variablename,variablevalue},
	FORMATREPS=Join[FORMATREPS,{"log"->"myLog"}];
	fortassign[variablename_,variablevalue_,file_]:=(Write[file,FortranAssign[variablename,variablevalue]]);
	OpenWrite[outfile];
