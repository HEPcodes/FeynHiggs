(*
  #****m* SecDec/loop/src/subexp/formContourPC.m
  #  NAME
  #    formContourPC.m
  #
  #  USAGE
...
formnumericalintegrandC[explab_,epsordreq_,0]:=
  Block[{condef,condeff,condef3,counter,epsord,helper,prods,mufunct,expnumfact,exorder,expnfeps,mu,expi,functionnumber},
	numberab[explab];
	Clear[integrand];
	Do[
	   If[ MatchQ[degen[explab,functionnumber],0]==False
	       ...
	       (*Do replacement of eps->epsilon in denominator 
		of numerator first and only then the 
		full deformation:*)
	       nstore[explab,functionnumber]=
	       (nstore[explab,functionnumber]/.{eps->epsilon}/.replrules);
	       ...
	       nstore[explab,functionnumber]=(nstore[explab,functionnumber]/.{eps->epsilon}/.replrules)*condef[[1]]*prods;
		...
		***********************)
	       ...
	       Do[If[epspower[mu]<=epsordreq
		     ,		     
		     mufunct={degen[explab,functionnumber]*funset[explab,functionnumber,dset[mu]]}; (*Form subtraction terms where one parameter is set to zero after having taken the derivative*)	    
		     ...
		     (****************************
		      Fill all terms after subtraction and eps expansion into integration list which is written into files in the next ParallelDo loop
		      ****************************)
	       ...
	       ParallelDo[
			  If[MatchQ[integrand[epsord],{}]==False
				...
				If[mathematicaflag==1, 
				   ...
				   resoutfile=StringJoin[direy,"/",ToString[counter],"x",pointname,ToString[epsord],".out"];
				   stream1 = OpenWrite[resoutfile, FormatType -> OutputForm];
				   $Messages = {stream1};
				   Do[lrs[i-1]=1,{i,feynpars}];
				   xvals[0]=1;
				   nintegtime=Timing[
				     result=NIntegrate[functiontooptimize,Evaluate[Sequence@@Table[{z[i],0,1},{i,feynpars}]],AccuracyGoal->2,Method->"AdaptiveMonteCarlo",WorkingPrecision->12];
				     Close[stream1];
						     ][[1]];
				   writeM[result,resoutfile,nintegtime];
				   Print["Integration of f",counter," in epsorder=",epsord," with NIntegrate done."]
				   ,
...
formnumericalintegrandC[explab_,epsordreq_,1]:=
  Block[{condef,condeff,condef3,prods,expt,exptifl,expdif,mufunctpre,mui,mu,inpa,varsset,mufunct,functionnumber},
	...
	Do[If[MatchQ[degen[explab,functionnumber],0]==False
	      ...
	      If[$VersionNumber==7,
		 SetSharedVariable[condeff,condef3,functionnumber];
		 SetSharedFunction[nstore,fstore,ustore];
		 ];
	      Clear[condef];
	      (*****END deformation of contour***********)
	      (*****BEGIN subtraction******)
...
EXPLICITIZE[Fmf_,emu_,eepsreq_,Tepsord_,Epsexpandedfunct_,postDpart_,gstoreEX_,gcondef3EX_]:=
  Block[{pDa,pDi,postDmf,TIMESmf,thisepsorder,thisepsexpandedfunct,explicitmf,explicitTIMES,epspole},
	...
	CriticalSection[{lock5},INTEGRANDIZE[#,emu,postDpart,explicitmf,explicitTIMES,gstoreEX,gcondef3EX]&/@thisepsexpandedfunct];
	Share[];
	Do[
	   ...
	   ,{epspole,Max[-minpole,epspower[emu]+mindegree],precisionrequired}
	   ];
	...
	If[ListQ[intlist],INTEGRANDIZE[#,imu,PostDpart,iexplicitmf,iexplicitTIMES,gstoreIN,gcondef3IN]&/@intlist;currentepsorder++,
	   ...
	   If[Or[mindegree!=0,maxdegree!=0],   
		 ...
		 If[And[MatchQ[integrands[epspower[imu],currentepsorder+ord],{}]==False,memcount[epspower[imu],currentepsorder+ord]+nextmemcount>MEMCUTOFF],
		    ...
		    WRITEFUNCTION[imu,currentepsorder+ord,gstoreIN,gcondef3IN];
		    ];
		 ...
		 WRITEFUNCTION[imu,currentepsorder,gstoreIN,gcondef3IN];
		 ];
...
ftoC[ffort_,fmu_,fepsreq_,nfort_,gstore_,gcondef3_,varsfort_]:=
  Block[{mf,thisepsorder,epsexpandedfunct,postDlist,postDpart,postDtemp,TIMESlist},
...
If[$VersionNumber==7,
   ...
   DistributeDefinitions[argexp,Cstring0,Cstring1,Cstring2,Cstring22,Cstring3,Cstring4,Cstring4a,Cstring5,Cstring6,Cstring7,Deformation,degen,der,direyt,dset,endpointflag,epsmulti,epspower,EXPLICITIZE,exponents,expf,expu,feynpars,ftoC,funset,genTau,integrandfunctionlist,INTEGRANDIZE,invarremap,invariantremapping,invmap,mathematicaflag,minpole,mindegree,maxdegree,modexp,ms,n,numcoeff,optimlambdafunC,oscillatory,partsflag,polestring,prestring,precisionrequired,ratexp,replrules,ssp,varletter,WRITEFUNCTION,writeoptC,writeM,xilist,z];
]
