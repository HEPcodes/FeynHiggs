...
int main (int argc, char **argv)
{
 ...
 const int nrfunc=16;
 ofstream results;
 for (int l=0; l<nrfunc;l++){ lambda[l]=4.01; lambdamax[l]=0.0; r[l]=0.0; mintest[l]=0.0;argtest[l]=100000.0; }
 ...
 if (esx[0]>=min(min(min(pow(sqrt(em[0])+sqrt(em[1]),2),pow(sqrt(em[0])+sqrt(em[2])+sqrt(em[3]),2)),pow(sqrt(em[1])+sqrt(em[2])+sqrt(em[3]),2)),4*em[3]))
 { 
 int evalopt = 8000;
 findoptlam (ndim, evalopt, nrfunc);
 run = 0;
 for (int i = 0; i < 16; i++ ) { if(mintest[i]==0) run++; }
 if (run>0) {
...
int Integrand4 (const double xx[],const int ncomp)
{
 ...
 f0[14][0]=Pt15t1(xx,es,esx,em,lambda[14],lrs[14],maxinv);
 f0[14][1]=Pt15t2(xx,es,esx,em,lambda[14],lrs[14],maxinv);
 f0[14][2]=Pt15t3(xx,es,esx,em,lambda[14],lrs[14],maxinv);
 f0[14][3]=Pt15t4(xx,es,esx,em,lambda[14],lrs[14],maxinv);
 f0[15][0]=Pt16t1(xx,es,esx,em,lambda[15],lrs[15],maxinv);
 f0[15][1]=Pt16t2(xx,es,esx,em,lambda[15],lrs[15],maxinv);
 f0[15][2]=Pt16t3(xx,es,esx,em,lambda[15],lrs[15],maxinv);
 f0[15][3]=Pt16t4(xx,es,esx,em,lambda[15],lrs[15],maxinv);
...
int Integrand3 (const double xx[], const int ncomp)
{
 ...
 f0[14]=Pm15(xx,es,esx,em,lambdatest[14][ipass],lrs[14],maxinv);
 f0[15]=Pm16(xx,es,esx,em,lambdatest[15][ipass],lrs[15],maxinv);
 ...
 s0[14]=Ps15(xx,es,esx,em,lambdatest[14][ipass],lrs[14],maxinv);
 s0[15]=Ps16(xx,es,esx,em,lambdatest[15][ipass],lrs[15],maxinv);
...
int Integrand2 (const double xx[], const int ncomp)
{
 ...
 f0[14]=Pr15(xx,es,esx,em,lambda[14],lrs[14],maxinv);
 f0[15]=Pr16(xx,es,esx,em,lambda[15],lrs[15],maxinv);
 for ( int i=0; i<ncomp; i++ )
 {
...
int Integrand (const int *ndim, const double x[], const int *ncomp, double f[], void *userdata)
{
 ...
 f0[14]=Pf15(x,es,esx,em,lambda[14],lrs[14],maxinv);
 f0[15]=Pf16(x,es,esx,em,lambda[15],lrs[15],maxinv);
 int ii =0;
 if (*(int*)userdata == 1)
 {
 f[0]=0.0;
  while (ii<16) { f[0]+=real(f0[ii]);ii++; }
 } else {
 f[0]=0.0;
  while (ii<16) { f[0]+=imag(f0[ii]);ii++; }
 }
 return 0;
}