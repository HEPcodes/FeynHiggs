...
int main (int argc, char **argv)
{
 ...
 const int nrfunc=10;
 ofstream results;
 for (int l=0; l<nrfunc;l++){ lambda[l]=3.1; lambdamax[l]=0.0; r[l]=0.0; mintest[l]=0.0;argtest[l]=100000.0; }
 ...
 if (esx[0]>=min(pow(sqrt(em[0])+sqrt(em[1]),2),pow(sqrt(em[1])+sqrt(em[2])+sqrt(em[3]),2)))
 { 
 int evalopt = 8000;
 findoptlam (ndim, evalopt, nrfunc);
 run = 0;
 for (int i = 0; i < 10; i++ ) { if(mintest[i]==0) run++; }
 if (run>0) {
...
int Integrand (const int *ndim, const double x[], const int *ncomp, double f[], void *userdata)
{
 ...
 {
 f[0]=0.0;
  while (ii<10) { f[0]+=real(f0[ii]);ii++; }
 } else {
 f[0]=0.0;
  while (ii<10) { f[0]+=imag(f0[ii]);ii++; }
 }
 return 0;
}