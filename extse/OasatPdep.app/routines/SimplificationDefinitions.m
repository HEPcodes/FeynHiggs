...
TrySimplify[x_] := Module[{time, ex, pr}, time = ByteCount[x]/500; pr = Null; 
      ...
      If[ex === x, ex = Factor[x]]; ex]], 10, x]]] (*100, x*)
...
SimplifySimpleProduct[x_] := If[Head[x] === Times && 
    ...
    res2 = TimeConstrained[Simplify /@ Apart[x], Min[ByteCount[x]/5000,3], x]; (*ByteCount[x]/5000*) 
...
TryTogether[x_] := ShorterOne[TimeConstrained[Together[x], 10, x], x] (*60, x],x*)
