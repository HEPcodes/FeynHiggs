...
\usepackage{amsmath,amssymb,multicol,mathpple}
...
\chapter*{FeynHiggs 2.15.0\\[2ex] Application Programming Interface}
...
\item[3:] NNLL resummation (for large $M_{\tilde\chi^{0,\pm}}$,
  $M_{\tilde g}$, $M_{\text{SUSY}}$)
  \textit{(recommended)},
\item[4:] NNLL resummation using THDM as EFT
  \textit{(recommended for $M_A \ll {M_\text{SUSY}}$)}.
\end{itemize}
...
\item[1:] 1L resummation,
\item[2:] 1L resummation for Higgs self-energies, 2L resummation for Higgs decays \textit{(recommended)}.
\end{itemize}

\FLAGIN{integer tlCplxApprox}{0}{3}
...
\item[0:] all corrections ($\alpha_s\alpha_t$, $\alpha_s\alpha_b$,
  $\alpha_t^2$, $\alpha_t\alpha_b$) are computed in the rMSSM
  \textit{(recommended for evaluation in the rMSSM)},
\item[1:] only the cMSSM $\alpha_s\alpha_t$ and $\alpha_t^2$ corrections
  are used,
\item[2:] the cMSSM $\alpha_s\alpha_t$ and $\alpha_t^2$ corrections are
  combined with the remaining corrections in the rMSSM,
\item[3:] the cMSSM $\alpha_s\alpha_t$ and $\alpha_t^2$ corrections are
  combined with the remaining corrections, interpolated in the complex
  phases (cf.\ \CODE{FHSelectIPol}).
\end{itemize}
...
\CODE{FHSetFlags} except that it takes a 10-character string as argument
rather than 10 integers.
...
\item[0:] no resummation,
\item[1:] 1L resummation,
\item[2:] 1L resummation for Higgs self-energies, 2L resummation for Higgs decays.
\end{itemize}

\FLAGOUT{integer tlCplxApprox}{0}{3}
determines how the two-loop corrections are treated in the presence of
complex parameters (cMSSM):
\begin{itemize}
\item[0:] all corrections ($\alpha_s\alpha_t$, $\alpha_s\alpha_b$,
...
10-character string as argument rather than 10 integers.
...
\VARIN{double precision invAlfa0, invAlfaMZ}
the value of electromagnetic coupling constant $\alpha^{-1}(0)$,
$\alpha^{-1}(M_Z)$, or $-1$ for the default value.
...
\VARIN{GammaW, GammaZ}
the W and Z widths, or $-1$ for the default values.
...
\VAROUT{double precision invAlfa0, invAlfaMZ}
the value of electromagnetic coupling constant $\alpha^{-1}(0)$,
$\alpha^{-1}(M_Z)$.
...
\VAROUT{double precision GammaW, GammaZ}
the W and Z widths.
...
\VAROUT{double precision DeltaAlfa}
...
\VAROUT{double precision AlfasMT}
the value of $\alpha_s$ at $m_t$.
...
\CODE{FHGetFV} returns the FV SUSY breaking parameters computed from the
...
\VAROUT{integer error}
zero if successful, otherwise the line number in \CODE{GetFV.F}
from which the error message was emitted.
...
\ROUTINE{FHGetSelf, FHGetSelfOL, FHGetSelfTL, FHGetSelfEFT, FHGetSelfgl}

\DESCRIPTION

\CODE{FHGetSelf} returns the renormalized Higgs self-energies at a given
$k^2$.  \CODE{FHGetSelfOL} (\CODE{FHGetSelfTL}) returns the one-loop
(two-loop) self-energies. \CODE{FHGetSelfEFT} return the corresponding
EFT correction. \CODE{FHGetSelfgl} does the same as \CODE{FHGetSelf}
but with results in the gaugeless limit.
The flags and parameters must have been set before
with \CODE{FHSetFlags} and \CODE{FHSetPara}.
...
FHGetSelfOL[k2, key, dkey, ren]
FHGetSelfTL[k2, key, dkey, ren]
FHGetSelfEFT[k2, key, dkey, ren]
...
\ROUTINE{FHSelectIpol}

\DESCRIPTION

\CODE{FHSelectIpol} selects the interpolation mode for the trilinear
coupling parameters.  Interpolation generally occurs when complex values
are chosen for $\mu$, $M_3$, $A_t$/$X_t$, or $A_b$/$X_b$, and
\CODE{FHSelectIpol} chooses between interpolation in $A_t$ vs $X_t$, and
$A_b$ vs $X_b$.
...
\VAROUT{integer error}
zero if successful, otherwise the line number in \CODE{SelectIpol.F}
from which the error message was emitted.

\VARIN{integer xt}
whether to interpolate in $A_t$ (0, default) or $X_t$ (1).

\VARIN{integer xb}
whether to interpolate in $A_b$ (0, default) or $X_b$ (1).
...
\CODE{FHEWPO} evaluates electroweak precision observables which can be
used as constraints on the MSSM parameter space.
...
\VAROUT{integer error}
zero if successful, otherwise the line number in \CODE{EWPO.F}
from which the error message was emitted.

\VAROUT{double precision Deltar}
the electroweak precision observable $\Delta r$.
...
\VAROUT{integer error}
zero if successful, otherwise the line number in \CODE{Flavour.F}
from which the error message was emitted.
...
\CODE{FHConstraints} evaluates further constraints, currently $(g_\mu -
2)$ and electric dipole moments.

\FSYNOPSIS

\begin{verbatim}
integer error, ccb
double precision gm2, edmeTh, edmn, edmHg

subroutine FHConstraints(error, gm2, edmeTh, edmn, edmHg, ccb)
\end{verbatim}
...
\VAROUT{integer ccb}
whether a colour-breaking minimum was detected.
